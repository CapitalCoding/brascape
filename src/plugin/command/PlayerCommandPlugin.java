...
public final class PlayerCommandPlugin extends CommandPlugin {
	...
	public boolean parse(Player player, String name, String[] arguments) {
		...
		case "home":
			if(player.inCombat()) {
				player.sendMessage("You can't use that command while under combat.");
				return true;
			}
			...
			player.getPulseManager().run(new Pulse(1, player) {
				...
				public boolean pulse() {
					switch(--counter){
					case -1:
						playerF.getInterfaceManager().closeChatbox();
						playerF.getTeleporter().send(ServerConstants.HOME_LOCATION, TeleportType.TELE_OTHER);
						return true;
					default:
						playerF.getDialogueInterpreter().sendPlainMessage(true, "You will be teleported in... "+counter+".", "Move away to cancel.");
						return false;
					}
				}
			});
