...
import org.wildscape.game.content.skill.member.summoning.pet.Pet;
import org.wildscape.game.content.skill.member.summoning.pet.PetDetails;
...
import org.wildscape.game.world.map.zone.ZoneRestriction;
...
public final class FamiliarManager implements SavingModule {
	...
	public void summon(Item item, boolean pet, boolean deleteItem) {
		...
		if (fam == null) {
			player.getPacketDispatch().sendMessage("Invalid familiar " + npcId + " - report on "+ServerConstants.WEBSITE_ADDRESS);
			return;
		}
	...
	private boolean summonPet(final Item item, boolean deleteItem, boolean morph, Location location) {
		...
		if (itemId > 8850 && itemId < 8900 && (!player.getName().equals("cabbage"))) {
			return false;
		}
		Pets pets = Pets.forId(itemId);
		if (pets == null) {
